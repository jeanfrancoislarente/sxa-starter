...
$sitecoreApiKey = ($envContent | Where-Object { $_ -imatch "^SITECORE_API_KEY_playsummit=.+" }).Split("=")[1]
...
Set-EnvFileVariable "NODEJS_VERSION" -Value $xmCloudBuild.renderingHosts.playsummit.nodeVersion
...
& docker\build\cm\templates\import-templates.ps1 -RenderingSiteName "playsummit" -SitecoreApiKey $sitecoreApiKey
